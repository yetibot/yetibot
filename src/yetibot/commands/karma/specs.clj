...
;; get-score
(s/def :yetibot.commands.karma.specs.get-score/match string?)
(s/def ::get-score-ctx
  (s/merge ::ctx (s/keys :req-un [:yetibot.commands.karma.specs.get-score/match])))


;; adjust-score
(s/def :yetibot.commands.karma.specs.adjust-score/match (s/and vector?
                                                               (s/cat :user-id ::user-id
                                                                      :action ::action
                                                                      :note (s/? ::note))))
(s/def ::adjust-score-ctx
  (s/merge ::ctx (s/keys :req-un [:yetibot.commands.karma.specs.adjust-score/match])))
