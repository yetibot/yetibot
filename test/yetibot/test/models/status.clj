...
(fact "about limiting statuses to given time periods"
  ...
  (against-background
    (before :facts (reset! statuses {123 (map status-map-for-n-days-ago (range 8))}))))

(fact "about sorting maps by a DateTime value within them"
  (let [sts-for-sorting {123 (map status-map-for-n-days-ago [1])
                         234 (map status-map-for-n-days-ago [2])
                         345 (map status-map-for-n-days-ago [0])}
        fs-sorted [[345 (flat-status-map-n-days-ago 0)]
                   [123 (flat-status-map-n-days-ago 1)]
                   [234 (flat-status-map-n-days-ago 2)]]]
